    ...
    class fuel_configuration:
        ...
        def compute_W_ff_cruise(self, obj, **kwargs):
            if self.n == 1:
                ...
                self.l1 = haversine_distance(obj.orig[0], obj.orig[1], x[0], x[1]);
                self.l2 = haversine_distance(A_other.orig[0], A_other.orig[1], x[0], x[1]);
                self.l_f = haversine_distance(A_other.dest[0], A_other.dest[1], x[0], x[1]);
                
                self.t1 = self.l1 / (obj.aircraft.V_cruise*3.6);
                self.t2 = self.l2 / (A_other.aircraft.V_cruise*3.6);
                self.V_f = min(obj.aircraft.V_cruise, A_other.aircraft.V_cruise)*3.6; # formation velocity
                
                # cruise alone 1
                cruise_alone_1 = round(compute_W_ff_cruise(self.l1, obj.aircraft.SFC_cruise, obj.aircraft.V_cruise*3.6, 0.866*obj.aircraft.L_D_max), 3);
        ...
        def stats(self, obj):
            ...
            if self.n == 1:
                ...
                # total distance in km
                self.distance_total = round(self.l1 + self.l_f,1)
...
#A1 = flight(equipment="A333", orig=[-73.567256, 45.5016889], dest=[-0.076132, 51.508530], W_payload=42000)
#A2 = flight(equipment="B789", orig=[-80.191788, 25.761681], dest=[-0.076132, 51.508530],  W_payload=40000)

#A1.equipment_params()
#A2.equipment_params()
#A1.compute_cruise_speed()
